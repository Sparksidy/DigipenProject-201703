...
void Application::Update(float frameTime)
{
  ...
  for(size_t i = 0; i < results.mResults.size(); ++i)
  {
    ...
    if(mRunGjk && gjk.Intersect(&shape0, &shape1, mMaxIterations, mDebuggingIndex, closestPoints, epsilon, mDrawGjk))
    {
...
void Application::SetBroadphaseType(const int& type)
{
  for(size_t i = 0; i < mGameObjects.size(); ++i)
  {
    ...
    Model* model = gameObject->has(Model);
    if(model != nullptr)
      mDynamicBroadphase->RemoveProxy(model->mProxyId);
  }
  ...
  if(type == SpatialPartitionTypes::NSquared)
    mDynamicBroadphase = new NSquaredSpatialPartition();
  if(type == SpatialPartitionTypes::NSquaredAabb)
    mDynamicBroadphase = new BoundingAabbSpatialPartition();  
  ...
  for(size_t i = 0; i < mGameObjects.size(); ++i)
  {
    ...
    if(model != nullptr)
    {
      ...
      mDynamicBroadphase->CreateProxy(model->mProxyId, data);
    }
  }
}
...
void Application::UpdateGameObject(GameObject* gameObject)
{
  ...
  if(model != nullptr)
  {
    ...
    data.mBoundingSphere = model->mBoundingSphere;
    data.mClientData = model;
    mDynamicBroadphase->UpdateProxy(model->mProxyId, data);
  }
}
