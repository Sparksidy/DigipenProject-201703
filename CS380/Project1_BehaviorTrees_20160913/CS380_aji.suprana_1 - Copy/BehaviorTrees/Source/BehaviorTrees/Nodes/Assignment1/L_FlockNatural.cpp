...
Status L_FlockNatural::OnEnter(NodeData *nodedata_ptr)
{
  ...
  randomSeparation = g_random.RangeFloat(1.0f,2.3f);
...
Status L_FlockNatural::OnUpdate(float dt, NodeData *nodedata_ptr)
{
  ...
  data->acceleration += cohesion * data->cohesionFactor * randomCohesion + separation * data->separationFactor + alignment * data->alignmentFactor * randomAlignment;
  float test = D3DXVec3Length(&data->acceleration);
  //speedup acceleration if difference between accelaration and velocity is too big
...
void L_FlockNatural::GetAllNeighbour(dbCompositionList& allGo, NodeData* nodedata_ptr,dbCompositionList& returnedNeighbour)
{
  ...
  for (auto go : allGo)
  {
    ...
    if (distance < nodedata_ptr->GetCustomData<FlockCustomData>()->detectionDistance)
    {
