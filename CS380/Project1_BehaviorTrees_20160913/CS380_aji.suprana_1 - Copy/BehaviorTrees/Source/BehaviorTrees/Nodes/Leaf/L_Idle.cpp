...
void L_Idle::SetRandomTimer(NodeData *nodedata_ptr, float min, float max)
{
	...
	customdata->m_timer = g_random.RangeFloat(1.0f, 1.2f);
}
...
Status L_Idle::OnUpdate(float dt, NodeData *nodedata_ptr)
{
  ...
  {
    if (!IsNear(allGo[i]->GetBody().GetPos(), allGo[i]->GetTargetPOS(),0.1))
    {
      customdata->allInPosition = false;
      break;
    }

    customdata->allInPosition = true;
  }

  if (!customdata->allInPosition)
  {
    return Status::BT_RUNNING;
  }
