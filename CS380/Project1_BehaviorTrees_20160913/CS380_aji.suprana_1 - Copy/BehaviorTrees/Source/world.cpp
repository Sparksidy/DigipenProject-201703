...
void World::Initialize( CMultiAnim *pMA, std::vector< CTiny* > *pv_pChars, CSoundManager *pSM, double dTimeCurrent )
{
	...
	for( int i=0; i<25; i++ )
	{
		...
		g_trees.Register(name, "FormationChanging");							// register agent to behavior tree
		g_trees.GetAgentData(name).InitialTinyCustomData(npc);		// initialize custom data for each tiny
	}

  //GameObject* npc = new GameObject(g_database.GetNewObjectID(), OBJECT_NPC, "separate");
  //D3DXVECTOR3 pos(0.0f, 0.0f, 0.0f);
  //npc->CreateBody(100, pos);
  //npc->CreateMovement();
  //npc->CreateTiny(pMA, pv_pChars, pSM, dTimeCurrent, 1.0f, 1.0f, 1.0f);	//Color if needed
  //g_database.Store(*npc);
  //g_trees.Register("separate", "Example");							// register agent to behavior tree
  //g_trees.GetAgentData("separate").InitialTinyCustomData(npc);		// initialize custom data for each tiny
	// delete game objects
