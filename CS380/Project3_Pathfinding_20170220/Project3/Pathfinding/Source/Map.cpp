	  ...
	  m_terrainOccupancyMap(0)
{
	...
	InitArray(m_terrainOccupancyMap);
}
...
void Map::Serialize(const std::wstring& filename)
{
	...
	assert(!m_width && !m_terrain && !m_terrainColor && 
		!m_terrainInfluenceMap && !m_terrainOccupancyMap && "Map may already have data");
...
void Map::Destroy()
{
	...
	DestroyArray(m_terrainOccupancyMap);
}
...
float **Map::GetOccupancyMap(void) const
{
	return m_terrainOccupancyMap;
}
