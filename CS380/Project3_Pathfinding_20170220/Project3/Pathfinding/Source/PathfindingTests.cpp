...
static void UpdateMap(int mapindex);
...
void MovementData::Set(Movement& movement)
{
	...
	m_fogOfWar = movement.GetFogOfWar();
	m_extracredit = movement.GetExtraCredit();
}

void MovementData::Apply(Movement& movement)
{
	...
	movement.SetFogOfWar(m_fogOfWar);
	movement.SetExtraCredit(m_extracredit);
}

// set pos data
void MovementData::SetPos(int row, int col)
{
	m_row = row;
	m_col = col;
...
// get pos data
void MovementData::GetPos(int &row, int &col)
{
	row = m_row;
	col = m_col;

	UpdateMap(m_mapindex);
}

void PathfindingTests::PrepareTest(Agent &agent, MovementData &movement_data,
...
void PathfindingTests::FinishTest(Agent &agent, MovementData &movement_data, bool applypos)
{
	Movement& movement = agent.m_owner->GetMovement();

	movement_data.Apply(movement);
	...
	std::string filename, Agent &agent, int type)
{
	...
	PrepareTest(agent, movement_data, 1, 1.01f, false);
	UpdateMap(mapIndex);
	...
	FinishTest(agent, movement_data, false);
}
...
void PathfindingTests::RunAllTests(const char *filename_noext, Agent &agent)
{
	...
	PrepareTest(agent, movement_data, heuristic, weight, false);

	int current_map = 1;
	UpdateMap(current_map);

	while (current_map != 0)
	{
		current_map = g_terrain.GetMapIndex();
	...
	bool is_straightline, bool is_rubberband, bool is_smooth, FileLoadSample sample)
{
	...
	UpdateMap(outcome.m_mapindex);
	PrepareTest(agent, movement_data, movement.GetHeuristicCalc(), movement.GetHeuristicWeight(), false);
