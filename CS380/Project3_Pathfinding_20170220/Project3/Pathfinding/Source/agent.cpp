...
BeginStateMachine
	...
	OnMsg(MSG_SetOccupancyMap)
		g_terrain.ResetOccupancyMap();
		...
		if (g_blackboard.GetOccupancyMapFlag())
		{
			...
			if (type == OccupancyMap_PropagationTwo ||
				type == OccupancyMap_PropagationTwoWithNormalizingInfluence)
			{
				GameObject *enemy = g_database.Find(g_database.GetIDByName("Enemy"));
				int row, col;
				D3DXVECTOR3 enemypos = enemy->GetBody().GetPos();
				g_terrain.GetRowColumn(&enemypos, &row, &col);
				g_terrain.InitialOccupancyMap(row, col, -1.0f);
			}
		}
