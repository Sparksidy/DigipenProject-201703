...
void Terrain::NextMap( void )
{
	...
	m_terrainOccupancyMap = map.GetOccupancyMap();
	m_timerUpdatePropagation = DEFAULT_UPDATEFREQUENCY;
...
void Terrain::ResetOccupancyMap(void)
{
	m_reevaluateAnalysis = true;
	...
	for (int r = 0; r<m_width; r++)
	{
		for (int c = 0; c<m_width; c++)
		{
			m_terrainOccupancyMap[r][c] = 0.0f;
		}
	}
}
...
void Terrain::DrawDebugVisualization( IDirect3DDevice9* pd3dDevice )
{
	...
	if (g_blackboard.GetTerrainAnalysisFlag() && (g_blackboard.GetTerrainAnalysisType() == TerrainAnalysis_Search))
	...
	for( int r=0; r<m_width; r++ )
	{
		for( int c=0; c<m_width; c++ )
		{
			...
			{
				if (Near(m_terrainOccupancyMap[r][c], 0.0f))
					continue;

				if (m_terrainOccupancyMap[r][c] < 0.0f)
				{
					D3DXCOLOR color = D3DXCOLOR(0.0f, 0.0f, 1.0f, -m_terrainOccupancyMap[r][c]);
					g_debugdrawing.DrawQuad(pd3dDevice, &pos, 1.0f / m_width, color);
				}
				else if (m_terrainOccupancyMap[r][c] > 0.0f)
				{
					D3DXCOLOR color = D3DXCOLOR(1.0f, 0.0f, 0.0f, m_terrainOccupancyMap[r][c]);
					g_debugdrawing.DrawQuad(pd3dDevice, &pos, 1.0f / m_width, color);
				}
			}
			else
...
{
	m_terrainOccupancyMap[row][col] = value;
	m_reevaluateAnalysis = false;
}
...
void Terrain::UpdateOccupancyMap(void)
{
	if (g_blackboard.GetOccupancyMapFlag())
	{
		OccupancyMapType type = static_cast<OccupancyMapType>(g_blackboard.GetOccupancyMapType());
		if (type == OccupancyMap_Propagation ||
			type == OccupancyMap_PropagationTwo ||
			type == OccupancyMap_PropagationWithNormalizingInfluence ||
			type == OccupancyMap_PropagationTwoWithNormalizingInfluence ||
			(type == OccupancyMap_HideAndSeek && g_blackboard.GetSeekPlayerFlag()))
		{
			if (m_timerUpdatePropagation < 0.0f)
			{
				if (type == OccupancyMap_PropagationTwo ||
					type == OccupancyMap_PropagationTwoWithNormalizingInfluence)
				...
				if (type == OccupancyMap_PropagationWithNormalizingInfluence ||
					(type == OccupancyMap_HideAndSeek && g_blackboard.GetSeekPlayerFlag()))
					NormalizeOccupancyMap(false);
				else if (type == OccupancyMap_PropagationTwoWithNormalizingInfluence)
