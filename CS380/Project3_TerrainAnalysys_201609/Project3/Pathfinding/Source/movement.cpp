  ...
  m_fogOfWar(false)
{
  ...
  m_goal = g_terrain.GetCoordinates(4, 5);
...
void Movement::Animate( double dTimeDelta )
{
  ...
  bool repath = g_terrain.UpdateFogOfWar(m_owner->GetBody().GetPos(), m_owner->GetBody().GetDir());

  if (repath)
  {
    D3DXVECTOR3 repathGoal = m_goal;
    int r, c;
    g_terrain.GetRowColumn(&repathGoal, &r, &c);
    ComputePath(r, c, true);
  }
