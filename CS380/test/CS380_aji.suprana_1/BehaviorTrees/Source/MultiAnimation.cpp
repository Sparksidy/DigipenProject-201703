...
CDXUTDialog             g_flockUI;
CDXUTDialog             g_ChangeTree;
...
void    CALLBACK OnGUIChangeTree(UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext);
...
void InitApp()
{
  ...
  g_flockUI.Init(&g_DialogResourceManager);
  g_ChangeTree.Init(&g_DialogResourceManager);
  ...
  g_flockUI.SetCallback(BT::FlockCustomData::OnGUIEvent);

  g_flockUI.AddStatic(IDC_COHESIONTXT, L"Cohesion", -60, iY += 25, 100, 20);
  g_flockUI.AddSlider(IDC_COHESION, -60, iY += 25, 200, 20,0, 100, 50);

  g_flockUI.AddStatic(IDC_ALIGNMENTTXT, L"Alignment", -60, iY += 25, 100, 20);
  g_flockUI.AddSlider(IDC_ALIGNMENT, -60, iY += 25, 200, 20, 0, 100, 50);

  g_flockUI.AddStatic(IDC_DETECTIONDISTANCETXT, L"detection Distance", -60, iY += 25, 100, 20);
  g_flockUI.AddSlider(IDC_DETECTIONDISTANCE, -60, iY += 25, 200, 20, 0, 100, 50);

  g_flockUI.AddStatic(IDC_SEPARATIONDISTANCETXT, L"separation Distance", -60, iY += 25, 100, 20);
  g_flockUI.AddSlider(IDC_SEPARATIONDISTANCE, -60, iY += 25, 200, 20, 0, 100, 50);

  g_HUD.SetCallback(OnGUIChangeTree); iY = 10;
  g_HUD.AddButton(IDC_CHANGEFLOCKING, L"Tree1: Flocking (1)", -200, iY += 24, 125, 22, '1');
  g_HUD.AddButton(IDC_CHANGEFOLLOWING, L"Tree2: FlockFollow(2)", -200, iY += 24, 125, 22, '2');
  g_HUD.AddButton(IDC_CHANGEFORMATION, L"Tree3: FormationChange(3)", -200, iY += 24, 125, 22, '3');
	...
	D3DXVECTOR3 vEye(.5f, 10.55f, -10.4f);
	...
	const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext)
{
  ...
  g_flockUI.SetLocation(pBackBufferSurfaceDesc->Width - 170,0);
  g_flockUI.SetSize(170, 220);
  g_ChangeTree.SetLocation(0, 0);
  g_ChangeTree.SetSize(170, 220);
...
void CALLBACK OnFrameRender(IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext)
{
    ...
    V(g_flockUI.OnRender(fElapsedTime));
    V(g_ChangeTree.OnRender(fElapsedTime));
...
LRESULT CALLBACK MsgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool* pbNoFurtherProcessing, void* pUserContext)
{
	...
	*pbNoFurtherProcessing = g_SampleUI.MsgProc(hWnd, uMsg, wParam, lParam);
	if (*pbNoFurtherProcessing)
		return 0;
  *pbNoFurtherProcessing = g_flockUI.MsgProc(hWnd, uMsg, wParam, lParam);
  if (*pbNoFurtherProcessing)
    return 0;
  *pbNoFurtherProcessing = g_ChangeTree.MsgProc(hWnd, uMsg, wParam, lParam);
  if (*pbNoFurtherProcessing)
    return 0;
...
//--------------------------------------------------------------------------------------
void TreeChangeHelper(const char* treeName)
{
  for (int i = 0; i<25; i++)
  {
    //Create game objects
    ...
    npc->CreateMovement();
    npc->CreateTiny(&g_MultiAnim, &g_v_pCharacters, &g_DSound, DXUTGetGlobalTimer()->GetTime());	//Color if needed
    g_database.Store(*npc);

    g_trees.Register(name, treeName);							// register agent to behavior tree
...
void    CALLBACK OnGUIChangeTree(UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext)
{
  switch (nControlID)
  {
  case IDC_CHANGEFLOCKING: TreeChangeHelper("Flocking"); break;
  case IDC_CHANGEFOLLOWING: TreeChangeHelper("FlockTailing"); break;
  case IDC_CHANGEFORMATION: TreeChangeHelper("FormationChanging"); break;
